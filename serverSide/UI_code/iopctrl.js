iopctrl = function() {
    ...
    iopctrl.led = function() {
        var width = 20, height = 10, round = 2, on = true, colorOn = d3v3.rgb("lightgreen"), colorOff = colorOn.darker(2), transitionDuration = 100;
        var _ledUpdate;
            ...
            g.each(function() {
                ...
                var led = g.selectAll("rect").data([ 0 ])
                    , ledUpdate = (led.enter().append("rect")
                        ...
                        .attr("height", height)
                        .attr("rx", round)
                        .style("fill", on ? colorOn : colorOff), d3v3.transition(led))
                    ledExit = d3v3.transition(led.exit()).style("opacity", 1e-6).remove();
    ...
    iopctrl.ledarray = function() {
        ...
        var scale = d3v3.scale.linear()
            .range([200, 0]);
        var thresholds = d3v3.scale.threshold()
            .domain([0.5, 0.8])
        ...
        function ledarray(g)
        {
            g.each(function() {
                ...
                var data = d3v3.range(r < 0 ? range[0] - _h : range[0], r < 0 ? range[1] - _h : range[1], delta);
                
                var ledarray = g.selectAll(".led").data(data)
                    , ledarrayUpdate = (ledarray.enter().append("g"), d3v3.transition(ledarray))
                    , ledarrayExit = d3v3.transition(ledarray.exit()).style("opacity", 1e-6).remove();
        ...
        function slider(g)
        {
            ...
            g.each(function() {
                ...
                var bar = g.selectAll(".bar").data([ 0 ])
                    , barUpdate = (bar.enter().append("g").attr("class", "bar"), d3v3.transition(bar));
                ...
                var b = g.selectAll(".band").data(bands)
                    , bandsUpdate = (b.enter().append("g").attr("class", "band"), d3v3.transition(b))
                    , bandsExit = d3v3.transition(b.exit()).style("opacity", 1e-6).remove();
                ...
                var a = g.selectAll(".axis").data([ 0 ])
                    , axisUpdate = (a.enter().append("g").attr("class", "axis"), d3v3.transition(a));
                ...
                var indicator = g.selectAll(".indicator").data([ 0 ])
                    , indicatorUpdate = (indicator.enter().append("g").attr("class", "indicator"), d3v3.transition(indicator));
                ...
                var touch = g.selectAll(".touch").data([ 0 ])
                    , touchUpdate = (touch.enter().append("g").attr("class", "touch"), d3v3.transition(touch));
        ...
        function addEvents(g, start, end)
        {
            g.on("selectstart", function() {
                ...
                .on("pointerdown", function() {
                    var x = d3v3.event.clientX, y = d3v3.event.clientY;
                ...
                .on("flick", function() {
                    if(enableFlick) {
                        var flickVelocity = d3v3.event.velocity;
                        var flickAngle = d3v3.event.angle * Math.PI / 180;
    ...
    iopctrl.arcslider = function()
    {
        ...
        function arcslider(g)
        {
            ...
            g.each(function() {
                ...
                var arc = g.selectAll(".arc").data([ 0 ])
                    , arcUpdate = (arc.enter().append("g").attr("class", "arc"), d3v3.transition(arc));
                ...
                bandsUpdate.attr("transform", "translate(" + (radius + margin.left) + ", " + (radius + margin.top) + ")")
                    .each(function(band) {d3v3.select(this).append("path")
                                .attr("class", band.class)
                                .attr("d", d3v3.svg.arc()
        ...
        function redraw(value, td)
        {
            ...
            _cursorUpdate.transition()
                ...
                .each(function() {
                    if(_comp == 0) {
                        d3v3.transition(_pointerUpdate)
                            .duration(td)
                            .delay(0)
                            .ease(ease)
    ...
    iopctrl.segdisplay = function() {
        ...
        function segdisplay(g)
        {
            g.each(function() {
                ...
                _dispUpdate = (disp.enter().append("g").attr("class", "digit"), d3v3.transition(disp));
        ...
        function litDigit(digit, val, dot) {
            ...
            digit.selectAll("*").each(function(d, i) {
                d3v3.select(this).attr("class", (i < 7 && cond[i]) || (i==7 && dot) ? "on" : "off");
            });
    ...
    iopctrl.arcaxis = function() {
        var scale = d3v3.scale.linear(), outerRadius = 100, innerRadius = 100, orient = "out", tickMajorSize = 6, tickMinorSize = 4, tickEndSize = 6, tickPadding = 3, tickArguments_ = [ 10 ], tickValues = null, tickFormat_, tickSubdivide = 0, normalize = true;
        function arcaxis(g) {
            g.each(function() {
                var g = d3v3.select(this);
                var ticks = tickValues == null ? scale.ticks ? scale.ticks.apply(scale, tickArguments_) : scale.domain() : tickValues
                    , tickFormat = tickFormat_ == null ? scale.tickFormat ? scale.tickFormat.apply(scale, tickArguments_) : String : tickFormat_;
                ticks = tickArguments_[0]<3 ? d3v3.extent(ticks) : ticks;
                ...
                var subticks = iopctrl_axisSubdivide(scale, ticks, tickSubdivide)
                    ...
                    , subtickExit = d3v3.transition(subtick.exit()).style("opacity", 1e-6).remove()
                    , subtickUpdate = d3v3.transition(subtick).style("opacity", 1);
                ...
                var tick = g.selectAll(".tick.major").data(ticks, String)
                    ...
                    , tickExit = d3v3.transition(tick.exit()).style("opacity", 1e-6).remove()
                    , tickUpdate = d3v3.transition(tick).style("opacity", 1), tickTransform;
                
                var extent = iopctrl_scaleExtent(scale)
                    , path = g.selectAll(".domain").data([ 0 ])
                    , pathUpdate = (path.enter().append("path").attr("class", "domain"), d3v3.transition(path));
                ...
                switch (orient) {
                    case "out":
                    {
                        ...
                        pathUpdate.attr("d", d3v3.svg.arc()
                            .startAngle(extent[0])
                            .endAngle(extent[1])
